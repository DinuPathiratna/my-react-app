import { Link } from "react-router-dom";

export default function Home() {
  return (
    <div className="home">

      <Link to="/ASG-01">
        <button>Assignment 1</button>
      </Link>

      <br />

      <Link to="/ASG-02">
        <button>Assignment 2</button>
      </Link>

      <br />

      <Link to="/ASG-03">
        <button>Assignment 3</button>
      </Link>

      <br />

      <Link to="/ASG-04">
        <button>Assignment 4</button>
      </Link>
      <br />

      <Link to="/ASG-05">
        <button>Assignment 5</button>
      </Link>
      <br />

      <Link to="/ASG-06">
        <button>Assignment 6</button>
      </Link>
      <br />
      <Link to="/ASG-07">
        <button>Assignment 7</button>
      </Link>

      <br />
      <Link to="/ASG-08">
        <button>Assignment 8</button>
      </Link>

      <br />
      <Link to="/ASG-09">
        <button>Assignment 9</button>
      </Link>
      <br />
      <Link to="/ASG-10">
        <button>Assignment 10</button>
      </Link>
      <br />
      <Link to="/ASG-11">
        <button>Assignment 11</button>
      </Link>
      <br />
      <Link to="/ASG-12">
        <button>Assignment 12</button>
      </Link>
      <br />
      <Link to="/ASG-13">
        <button>Assignment 13</button>
      </Link>
      <br />
      <Link to="/ASG-14">
        <button>Assignment 14</button>
      </Link>
      <br />
      <Link to="/ASG-15">
        <button>Assignment 15</button>
      </Link>
      <br />
      <Link to="/ASG-16">
        <button>Assignment 16</button>
      </Link>
      <br />
      <Link to="/ASG-17">
        <button>Assignment 17</button>
      </Link>
      <br />
      <Link to="/ASG-18">
        <button>Assignment 18</button>
      </Link>
      <br />
      <Link to="/ASG-19">
        <button>Assignment 19</button>
      </Link>
      <br />
      <Link to="/ASG-20">
      <button>Assignment 20</button>
      </Link>
      <br />
      <Link to="/ASG-21">
        <button>Assignment 21</button>
      </Link>
      <br />
      <Link to="/ASG-22">
        <button>Assignment 22</button>
      </Link>
      <br />
      <Link to="/ASG-23">
        <button>Assignment 23</button>
      </Link>
      <br />
      <Link to="/ASG-24">
        <button>Assignment 24</button>
      </Link>
      <br />
      <Link to="/ASG-25">
        <button>Assignment 25</button>
      </Link>
      <br />
      <Link to="/ASG-26">
        <button>Assignment 26</button>
      </Link>
      <br />
      <Link to="/ASG-27">
        <button>Assignment 27</button>
      </Link>
      <br />
      <Link to="/ASG-28">
        <button>Assignment 28</button>
      </Link>
      <br />
      <Link to="/ASG-29">
        <button>Assignment 29</button>
      </Link>
      <br />
      <Link to="/ASG-30">
        <button>Assignment 30</button>
      </Link>
      <br />
      <Link to="/ASG-31">
        <button>Assignment 31</button>
      </Link>
    </div>
  );
}